...
router.get("/allergy-incidence", async (req, res, next) => {
  ...
  if (key) {
    const statsRes = await encounterStatsController.getAllergyIncidenceByKey(
      key
    );
    res.send(statsRes);
    return;
  ...
  const statsRes =
    await encounterStatsController.getAllergyIncidenceBetweenKeys(
      startKey,
      endKey
    );
  res.send(statsRes);
...
router.get("/patient-evolution", async (req, res, next) => {
  ...
  if (key) {
    const statsRes = await encounterStatsController.getPatientEvolutionByKey(
      key
    );
    res.send(statsRes);
    return;
  ...
  const statsRes =
    await encounterStatsController.getPatientEvolutionBetweenKeys(
      startKey,
      endKey
    );
  res.send(statsRes);
